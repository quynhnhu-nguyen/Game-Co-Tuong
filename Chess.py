...
class chess:
    def __init__(self, belong, chess_type):
        self.belong = belong  # Side's turn which
        self.chess_type = chess_type  # Chess piece which

    def can_move(self, to_x, to_y):  # Replay
        return False

class step:  # Move turn
...
class chess_board:
    def __init__(self):
        ...
        for i in range(9):
            self.board.append([])
            for j in range(10):
                if(c.init_borad[i][j] == 0):
                    self.board[i].append(chess(-1, c.init_borad[i][j]))
                else:
                    self.board[i].append(chess(0 if j < 5 else 1, c.init_borad[i][j]))
    ...
    def IsKingFaceToFace(self, x, y, who):
        ...
        if who == 1:
            for i in range(9, 6, -1):
                if self.board[x][i].chess_type == c.general:
                    a = True
                    b = i
            if not a:
                return a
            for j in range(y + 1, b):
                if self.board[x][j].chess_type != c.invalid:
                    return False
            return b
        else:
            for i in range(0, 3):
    ...
    # Returns all possible moves, a list of step types
    ...
    def get_chess_move(self, x, y, who, tag=False):
        ...
        if self.board[x][y].chess_type == 1 and who == 0:
            ...
            # x2, y2 is used to judge whether there is lame
            x2 = x + 1
            y2 = y
            if x2 <= 7 and (not self.HaveMan(x2, y2)):
                # Sideways up to the right
                y1 = y + 1
                x1 = x + 2
                if y1 <= 9 and (not self.HaveFriend(x1, y1, who)):
                    s = step(x, y, x1, y1)
                    list3.append(s)
                # Sideways down to the right
                y1 = y - 1
                x1 = x + 2
                if y1 >= 0 and (not self.HaveFriend(x1, y1, who)):
                    s = step(x, y, x1, y1)
            ...
            # x2, y2 is used to determine whether there is a crappy
            x2 = x - 1
            y2 = y
            if x2 >= 1 and (not self.HaveMan(x2, y2)):
                # Sideways up to the right
